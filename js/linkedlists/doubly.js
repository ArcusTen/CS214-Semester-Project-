...
function add_listeners() {
    ...
    document.querySelector(".arr__remove").addEventListener("click", () => {
...
function delete_animation() {
    if (ll.length === 1) {
        alert("Linked List has no element present to be deleted");
    } else {
        ll.splice(1, 1);
        createSinglyLinkedList(ll);
    }
...
function remove_middle_animation() {
    if (ll.length <= 2) {
        alert("No middle element to remove");
        return;
    }
    let middleIndex = Math.floor((ll.length - 1) / 2);
